...
function DrawText3Dsas(x,y,z, text)
    local onScreen,_x,_y=World3dToScreen2d(x,y,z)
    local px,py,pz=table.unpack(GetGameplayCamCoords())
...
Citizen.CreateThread(function() -- pump names.
	...
	while true do
		...
		if name then
			...
			AddTextComponentString(name.." "..tostring(stationFuel[stationId]).."l.")
			DrawText3Dsas(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'],"yes")
		end
		repeat
			Wait(0)
		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Islipk is tr.priemones")
				elseif IsFueling then
					...
					if pumpIsEmpty or IsControlJustReleased(0, 47) then
						if pumpIsEmpty then
							pumpIsEmpty = false
							exports.pNotify:SendNotification({text = "Šioje degalinėje nebėra kuro!",type = "error",timeout = 10000,layout = "centerRight",queue = "left"})
						end
				...
				elseif cash <= 0 then
					...
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Paspausk ~g~G ~w~kad pradetum pilti degalus. ~r~3.15~w~€/už litrą")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Paspausk ~g~G ~w~kad piltum degalus")
	...
	while true do
		...
		if vehicle and engine then
			...
			local rpmfuelusage = 0
			 ...
			 if rpm > 0.9 then
                rpmfuelusage = fuel - rpm / 9.97
				--print("top")
                Citizen.Wait(1000)
            elseif rpm > 0.8 then
                rpmfuelusage = fuel - rpm / 10.1
                Citizen.Wait(1500)
            elseif rpm > 0.7 then
                rpmfuelusage = fuel - rpm / 10.2
                Citizen.Wait(2000)
            elseif rpm > 0.6 then
                rpmfuelusage = fuel - rpm / 10.3
                Citizen.Wait(3000)
            elseif rpm > 0.5 then
			--print("middle")
                rpmfuelusage = fuel - rpm / 10.4
                Citizen.Wait(4000)
            elseif rpm > 0.4 then
			--print("bottom1")
                rpmfuelusage = fuel - rpm / 10.5
                Citizen.Wait(5000)
            elseif rpm > 0.3 then
                rpmfuelusage = fuel - rpm / 10.5
				--print("bottom2")
...
Citizen.CreateThread(function()
	if Config.EnableBlips then
		for k, v in ipairs(gas_stations) do
			...
			AddTextComponentString("Degaline")
